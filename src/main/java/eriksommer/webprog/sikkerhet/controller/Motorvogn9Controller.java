package eriksommer.webprog.sikkerhet.controller;
...
import eriksommer.webprog.sikkerhet.repository.Motorvogn9Repository;
import eriksommer.webprog.sikkerhet.service.Inputvalidering;
...
@RestController
@RequestMapping("/api/sikkerhet/")
public class Motorvogn9Controller {

    @Autowired
    private Motorvogn9Repository rep;

    @Autowired
    private HttpSession session;

    private final Logger logger = LoggerFactory.getLogger(Motorvogn9Controller.class);
    ...
    public void registerBruker(Bruker bruker, HttpServletResponse res) throws IOException {
        if (!rep.registrerBruker(bruker)) {
            res.sendError(500, "Feilet å lage kunde");
        }
    }
