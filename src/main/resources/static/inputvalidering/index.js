import {apiInputvalidering as api} from "../apiurl.js"

$(() => {  // kjøres når dokumentet er ferdig lastet
    hentAlle();

    $("#slettAlle").click(() => $.get(api + "/slettAlle", () => hentAlle()));
});
...
const idTilEndring = id => window.location.href = "/inputvalidering/endre.html?" + id;

const slettEnMotorvogn = personnr => {
    const url = "/slettEnMotorvogn?personnr=" + personnr;
    $.get(api + url, () => window.location.href = "/inputvalidering/index.html");
}